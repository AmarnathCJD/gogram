...
// normalizeBotStickerShortName enforces Telegram's requirement that sticker
// pack short_names created by a bot end with "_by_<botusername>" (lowercase).
// When the current account is a bot it auto-appends the suffix if missing.
// For user accounts the short_name is used as-is.
func (c *Client) normalizeBotStickerShortName(shortName string) (string, error) {
...
func (c *Client) CreateStickerSet(owner any, title, shortName string, stickers []StickerInput, opts ...*CreateStickerSetOptions) (MessagesStickerSet, error) {
	...
	finalShortName, err := c.normalizeBotStickerShortName(shortName)
	if err != nil {
		return nil, err
	}
